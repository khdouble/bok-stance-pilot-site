#!/usr/bin/env python3
"""Build the R5 public instrument from its locked, key-free source file."""
...
SOURCE_PATH = REPO_ROOT / "instrument_sources" / "r5_20260908.json"
...
RELEASE_SOURCE_PATHS = {
    "r5_source": Path("instrument_sources/r5_20260908.json"),
...
def load_source(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(source, dict):
        raise ValueError("R5 source must be an object")
    required = {
    ...
    if set(source) != required:
        raise ValueError("R5 source field set changed")
    if source["schema_version"] != "1.0":
        raise ValueError("R5 source schema version changed")
    if not re.fullmatch(r"v\d{6}-r5-public-\d+", str(source["hosted_version"])):
        raise ValueError("R5 hosted version has an invalid format")
    if not re.fullmatch(r"[0-9a-f]{64}", str(source["source_offline_instrument_sha256"])):
        raise ValueError("R5 source hash must be a SHA-256 digest")
    if (
    ...
    if not isinstance(provenance, dict) or set(provenance) != {
        ...
        raise ValueError("R5 provenance field set changed")
    ...
    if not isinstance(core_rows, list) or len(core_rows) != 12:
        raise ValueError("R5 must contain exactly 12 core items")
    ...
    if not isinstance(tutorial_items, list) or len(tutorial_items) != 6:
        raise ValueError("R5 must contain exactly six unscored tutorial items")
...
def build(source_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "source_hashes": {"r5_source": sha256_file(source_path)},
